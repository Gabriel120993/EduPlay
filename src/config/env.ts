...
const authRateLimitWindowMs = Math.max(60_000, Number(process.env.AUTH_RATE_LIMIT_WINDOW_MS) || 15 * 60 * 1000);
const authRateLimitMax = Math.max(5, Number(process.env.AUTH_RATE_LIMIT_MAX) || 25);

const apiRateLimitWindowMs = Math.max(10_000, Number(process.env.API_RATE_LIMIT_WINDOW_MS) || 60_000);
...
const apiChildUserRateLimitMax = Math.max(20, Number(process.env.API_CHILD_USER_RATE_LIMIT_MAX) || 280);
...
const apiParentUserRateLimitMax = Math.max(50, Number(process.env.API_PARENT_USER_RATE_LIMIT_MAX) || 900);
