...
const EMPTY_CONTENT_SUGGESTION = "Probá explorar otras categorías mientras preparamos nuevas actividades.";

const CATEGORY_META: Record<string, { icon: string; color: string; description: string }> = {
  math: { icon: "🔢", color: "#4F46E5", description: "Números, operaciones, geometría y desafíos matemáticos." },
  science: { icon: "🧪", color: "#059669", description: "Cuerpo humano, naturaleza, experimentos y sistema solar." },
  geography: { icon: "🌍", color: "#D97706", description: "Mapas, países, culturas, historia y mundo social." },
  education: { icon: "📖", color: "#DC2626", description: "Lectura, gramática, vocabulario y comprensión." },
  creativity: { icon: "🎨", color: "#DB2777", description: "Arte, música, teatro, expresión y cultura." },
  puzzle: { icon: "🧩", color: "#7C3AED", description: "Acertijos, patrones, lógica y razonamiento espacial." },
  astronomy: { icon: "🚀", color: "#2563EB", description: "Planetas, estrellas, exploración espacial y universo." },
  history: { icon: "🏺", color: "#B45309", description: "Civilizaciones, fuentes, líneas de tiempo y patrimonio." },
  sports: { icon: "⚽", color: "#16A34A", description: "Movimiento, coordinación, hábitos saludables y juego." },
};
...
function serializeContent(item: EducationalContent, progress: ProgressInfo | null = null): ContentCard {
...
function scoreContent(
  ...
  input: { interests: string[]; preferredDifficulty: Difficulty | null; startedIds: Set<string>; popularCategories: Set<string> },
): number {
  let score = 0;
...
export async function getContentFeed(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const started = recommendations
      ...
      .sort((a, b) => Date.parse(b.progress?.lastSeenAt ?? "") - Date.parse(a.progress?.lastSeenAt ?? ""));
...
export async function getContentExplore(req: Request, res: Response): Promise<void> {
  ...
  try {
    const [recommendationsResult, quizzes, games, visualGames, learn] = await Promise.all([
      recommendedContent({ userId, interests: parseStringListQuery(req.query.interests), limit: 12 }),
...
export async function getRecommendedContent(req: Request, res: Response): Promise<void> {
  ...
  if (req.query.type != null && !type) {
    res.status(400).json({ error: "type debe ser VIDEO, READING, EXPERIMENT, INTERACTIVE, WORKSHEET o AUDIO." });
    return;
  }
  ...
  try {
    ...
    const rows = categories.map((category) => {
      const contentCount = category.subjects.reduce(
        (sum, subject) => sum + subject.topics.reduce((topicSum, topic) => topicSum + topic._count.contents, 0),
        0,
      );
...
export async function getContentDetailWithProgress(req: Request, res: Response): Promise<void> {
  ...
  try {
    const item = await prisma.educationalContent.findUnique({
      where: { id: contentId },
      include: { topic: { include: { subject: { include: { category: true } } } }, heroImageAsset: true },
    });
    ...
    const related = await prisma.educationalContent.findMany({
      where: {
        published: true,
        id: { not: item.id },
        OR: [{ topicId: item.topicId }, { category: item.category }, { difficulty: item.difficulty }],
      },
      orderBy: { createdAt: "desc" },
      take: 8,
    });
    ...
    res.json({
      ...
      related: related.filter((row) => !isPlaceholderContent(row)).map((row) => serializeContent(row, progressById.get(row.id) ?? null)),
    });
  } catch (err) {
...
export async function postContentProgress(req: Request, res: Response): Promise<void> {
  ...
  const timeSpentSeconds = numericMetadataValue(req.body?.timeSpentSeconds ?? req.body?.timeViewedSeconds, 0);
