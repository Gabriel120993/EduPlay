...
export async function postForgotPassword(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (!parent) {
      res.json({
        ok: true,
        message: "Si el email está registrado, recibirás instrucciones para restablecer la contraseña.",
      });
      return;
    }
...
export async function postResetPassword(req: Request, res: Response): Promise<void> {
  ...
  try {
    const decoded = jwt.verify(parsed.data.token, env.jwtSecret, { algorithms: ["HS256"] }) as jwt.JwtPayload;
