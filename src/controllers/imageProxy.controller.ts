...
function pickSourceUrl(
  asset: { urlSmall: string; urlMedium: string; urlLarge: string },
  size: Size,
): string {
...
async function fetchOnce(
  url: string,
): Promise<{ buf: Buffer; ext: string } | { error: string; status: number }> {
...
async function downloadAndCache(
  ...
  if (
    'error' in attempt &&
    (attempt.status === 404 || attempt.status === 400 || attempt.status === 403)
  ) {
