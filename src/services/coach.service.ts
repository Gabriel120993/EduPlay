...
const DEFAULT_ACTIVITIES_TOGETHER: CoachActivityIdea[] = [
  {
    ...
    offlineHint:
      'Sin pantallas durante la actividad; después podés buscar en EduPlay un tema relacionado.',
  },
  {
    childId: '__default_walk',
    childName: 'Tu familia',
    learnedThisWeek:
      'Conectar el aprendizaje en pantalla con el mundo real mejora retención y motivación.',
    activity:
  ...
  },
  {
    ...
    offlineHint:
      'Si la frustración aparece, bajá el objetivo: solo medir 2 ingredientes ya está bien.',
  },
];
...
function activityFromCategory(cat: ContentCategory | null): {
  learned: string;
  activity: string;
  hint: string;
} {
  if (cat === ContentCategory.astronomy) {
    return {
      ...
      activity:
        'Salir a observar estrellas o la Luna con binoculares o a ojo; dibujar lo que vieron.',
  ...
  if (cat === ContentCategory.math) {
    return {
      ...
      activity:
        'Cocinar juntos midiendo ingredientes con taza y cuchara (fracciones en la vida real).',
  ...
  if (cat === ContentCategory.creativity) {
    return {
      ...
      activity:
        'Collage con materiales reciclados inspirado en un personaje o tema que le guste en la app.',
  ...
  if (cat === ContentCategory.science) {
    return {
      ...
      activity:
        'Experimento simple en casa: agua + sal + huevo o plantar semillas y anotar cambios cada día.',
  ...
  return {
    ...
    activity:
      'Paseo corto relacionado con lo último que jugó: contar 10 cosas que observan y clasificarlas.',
...
async function loadChildFeatures(parentId: string): Promise<ChildFeatures[]> {
  ...
  for (const c of children) {
    const [gamesThis, gamesPrev, xpThisAgg, xpPrevAgg, topInterest, mathResults] =
      await Promise.all([
...
function buildAlerts(features: ChildFeatures[]): CoachAlert[] {
  ...
  if (alerts.length === 0) {
    alerts.push({
      ...
      severity: 'info',
      message:
        'No detectamos cambios bruscos preocupantes esta semana. Seguí con el ritmo y revisá el reporte semanal.',
    });
  }
  return alerts;
}
...
export async function buildParentCoachPayload(parentId: string): Promise<ParentCoachPayload> {
  ...
  const interestLabel =
    features[0]?.topCategory != null
      ? categoryLabel(features[0].topCategory)
      : 'un tema que le entusiasma';
  ...
  return {
    ...
    activitiesTogether:
      features.length > 0 ? buildActivities(features) : DEFAULT_ACTIVITIES_TOGETHER,
