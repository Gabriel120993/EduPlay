...
export type ThematicMissionFestivity = "none" | "christmas" | "halloween" | "earth_day" | "carnival";
...
function m(
  ...
  if (steps.length < 5 || steps.length > 10) {
    throw new Error(`Misión ${slug}: se requieren entre 5 y 10 actividades (tiene ${steps.length}).`);
  }
  return {
    slug,
    title,
