...
export const DAILY_CHALLENGE_POOL: GamifiedChallengeDef[] = [
  ...
  },
  {
    ...
    title: "Explorador",
    description: "Completar 1 actividad en una materia que no hayas practicado esta semana (registrado por la app).",
    target: 1,
    rewardCoins: 20,
    rewardXp: 40,
    premiumTempHours: 0,
  ...
  },
  {
    slug: "speedster_30s",
    title: "Velocista",
    description: "Responder 3 preguntas correctas en menos de 30 segundos en total (sumatorio de tiempos entre respuestas).",
    target: 3,
    rewardCoins: 30,
    rewardXp: 50,
    premiumTempHours: 0,
  ...
  },
  {
    ...
    description: "Obtener 3 estrellas en cualquier minijuego o actividad con calificación por estrellas.",
    target: 1,
    rewardCoins: 25,
    rewardXp: 45,
    premiumTempHours: 0,
...
export const WEEKLY_CHALLENGE_POOL: GamifiedChallengeDef[] = [
  ...
  },
  {
    slug: "avid_reader_5",
    title: "Lector Ávido",
    description: "Leer 5 textos de comprensión lectora (contenido educativo marcado como aprendido).",
    target: 5,
    rewardCoins: 90,
    rewardXp: 150,
    premiumTempHours: 4,
  ...
  return [
    {
      ...
      description: "30 días de actividad consecutiva en EduPlay (la app envía incrementos diarios).",
      target: 30,
      rewardCoins: 500,
      rewardXp: 600,
      premiumTempHours: 48,
    ...
    },
    {
      ...
      description: "Competencia entre países: sumá puntos representando a tu país en quizzes globales.",
      target: 50,
      rewardCoins: 400,
      rewardXp: 500,
      premiumTempHours: 24,
...
export function activeSpecialWindows(now: Date = new Date()): SpecialChallengeWindow[] {
  const t = now.getTime();
  return listSpecialChallengeWindows(now).filter((w) => t >= w.startsAt.getTime() && t < w.endsAt.getTime());
}
