...
export async function requireMinor(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (!auth || auth.kind !== 'child') {
    res
      .status(403)
      .json({ error: 'Esta operación es solo para menores.', code: 'FORBIDDEN_NOT_MINOR' });
    return;
  }
...
export async function requireAdmin(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    const typ =
      decoded.typ === 'admin'
        ? 'admin'
        : (decoded as { userType?: string }).userType === 'admin'
          ? 'admin'
          : null;
...
export function requireParentOf(minorIdParam = 'minorId') {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!auth || auth.kind !== 'parent') {
      res.status(403).json({
        error: 'Solo un tutor puede acceder a este recurso.',
    ...
    const minorId =
      typeof req.params[minorIdParam] === 'string' ? req.params[minorIdParam].trim() : '';
    if (!minorId) {
    ...
    try {
      ...
      if (minor.parentId !== auth.parentId) {
        res
          .status(403)
          .json({ error: 'No autorizado para este menor.', code: 'FORBIDDEN_NOT_PARENT_OF_MINOR' });
        return;
      }
      next();
    } catch (e) {
...
export function requireSelfOrParent(userIdParam = 'userId') {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const targetId =
      typeof req.params[userIdParam] === 'string' ? req.params[userIdParam].trim() : '';
    if (!targetId) {
    ...
    if (auth.kind === 'child') {
      if (auth.userId !== targetId) {
        res
          .status(403)
          .json({ error: 'Solo podés acceder a tu propio perfil.', code: 'FORBIDDEN_NOT_SELF' });
        return;
      }
      next();
      return;
  ...
  try {
    ...
    const blocked =
      ...
      'blockedCategories' in row.minorProfile.contentRestrictions
        ? (row.minorProfile.contentRestrictions as { blockedCategories?: unknown })
            .blockedCategories
        : null;
    ...
    if (category && blockedCategories.includes(category)) {
      res.status(403).json({
        error: 'Categoría bloqueada por controles parentales.',
        code: 'CONTENT_CATEGORY_BLOCKED',
      });
      return;
    }
...
function mapLegacyActivityType(
  action: 'make_purchase' | 'post_content' | 'add_friend',
): ActivityType {
  switch (action) {
    case 'make_purchase':
...
export function requireApprovalFor(action: 'make_purchase' | 'post_content' | 'add_friend') {
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const profile =
        row && row.minorProfile && typeof row.minorProfile === 'object'
          ? (row.minorProfile as Record<string, unknown>)
          : null;
      if (action === 'make_purchase' && profile?.canMakePurchases === false) {
        res
          .status(403)
          .json({ error: 'Tu perfil no permite compras.', code: 'PURCHASES_DISABLED' });
        return;
      }
      ...
      if (action === 'add_friend' && profile?.canAddFriends === false) {
        res
          .status(403)
          .json({ error: 'Tu perfil no permite agregar amistades.', code: 'FRIENDS_DISABLED' });
        return;
      }
    } catch (e) {
