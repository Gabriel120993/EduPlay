...
describe('Auth integration (app por defecto)', () => {
...
/**
 * App aislada con límite estricto de login (1 req / ventana) para no contaminar el resto de la suite.
 */
describe('Auth integration — rate limit login', () => {
  ...
  it('aplica rate-limit en login/register tras 1 intento', async () => {
    ...
    const first = await request(appStrict).post('/auth/login').send({
      email: 'parent@example.com',
      password: 'secreto123',
    });
    expect(first.status).toBe(200);

    const second = await request(appStrict).post('/auth/login').send({
      email: 'parent@example.com',
      password: 'secreto123',
    });
